...
async function createEvent (eventDetails: EventDetails): Promise<Event | null> {
  ...
  let result: any; // fix these any to be real x TS
  try {
    const fn: any = httpsCallable(functions, 'createEvent');
    result = await fn({ // TODO: Add typescript annotations
      title: "nick ribs",
      description: "ymeets on top",
    });
  } catch(e) {
    console.error(e);
