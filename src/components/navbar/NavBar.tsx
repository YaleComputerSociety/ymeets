...
import { FaInfoCircle, FaCalendarAlt, FaSignInAlt, FaSignOutAlt } from 'react-icons/fa';
                ...
                <div className="flex bg-white rounded-xl h-16 w-[90%] px-5 sm:px-8 items-center justify-between shadow-lg">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="relative">
                            ...
                            {menuState !== 'closed' && (
                                <div className={`absolute z-10 mt-2 w-44 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 right-0 transition-all duration-200 ease-out transform origin-top-right 
                                    ...
                                    <div className="py-1 border border-gray-400 rounded-lg" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                        <a href="#" className="flex items-center justify-start px-4 py-2 text-sm text-gray-700 hover:bg-blue-500 hover:text-white transition-colors duration-200" onClick={() => { nav("/about-us"); setMenuState('closed'); }}>
                                            <FaInfoCircle className="mr-2" /> About Us
                                        </a>
                                        <div className="border-t border-gray-200"></div>
                                        <a href="#" className="flex items-center justify-start px-4 py-2 text-sm text-gray-700 hover:bg-blue-500 hover:text-white transition-colors duration-200" onClick={() => { nav("/useraccount"); setMenuState('closed'); }}>
                                            <FaCalendarAlt className="mr-2" /> Events
                                        </a>
                                        <div className="border-t border-gray-200"></div>
                                        {checkIfLoggedIn() ? (
                                            <a href="#" className="flex items-center justify-start px-4 py-2 text-sm text-gray-700 hover:bg-blue-500 hover:text-white transition-colors duration-200" onClick={() => { logout(gapi); setMenuState('closed'); }}>
                                                <FaSignOutAlt className="mr-2" /> Logout
                                            </a>
                                        ) : (
                                            <a href="#" className="flex items-center justify-start px-4 py-2 text-sm text-gray-700 hover:bg-blue-500 hover:text-white transition-colors duration-200" onClick={() => {
