...
import { getEventOnPageload, getEventName, getParticipantIndex, getAccountId, getEventDescription, getLocationsVotes, getLocationOptions, setChosenLocation, getChosenDayAndTime, updateAnonymousUserToAuthUser, getAccountName, setChosenDate, getChosenLocation } from '../../firebase/events'
...
export default function AdminGroupViewPage () {
  ...
  function handleSelectionSubmission () {
    ...
    // @ts-expect-error
    const calDate = [].concat(...calendarFramework.dates)[dragState.dragStartedOnID[0]]
    ...
    if (dragState.dragStartedOnID.length > 0 && dragState.dragEndedOnID.length > 0) {
      ...
      // @ts-expect-error
      const [startHour, startMinute] = selectedStartTimeHHMM.split(':').map(Number)
      // @ts-expect-error
                      ...
                      <div className = "hidden mb-4 flex flex-col space-y-5 md:block">
                          ...
                          <div className="flex flex-col">
                            <h3 className="text-base text-center md:text-left">
                              <span className='font-bold'>Time:</span> {selectedDateTimeObjects !== undefined
