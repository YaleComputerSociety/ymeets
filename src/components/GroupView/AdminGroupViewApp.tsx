...
export default function AdminGroupViewPage() {
  ...
  function handleSelectionSubmission() {
    ...
    //@ts-ignore
    let calDate = [].concat(...calendarFramework.dates)[
    ...
    if (
      ...
      //@ts-ignore
      let [startHour, startMinute] = selectedStartTimeHHMM
        .split(":")
        .map(Number);
      //@ts-ignore
          ...
          <div className="flex flex-col content-center space-y-7 flex-none md:w-[32%] mb-5 md:content-start md:mt-0">
            ...
            {!selectedDateTimeObjects && (
              <div className="p-1 w-[80%] text-gray-500 text-left text-sm md:text-left">
                NOTE: Click and drag as if you are selecting your availability
                to select your ideal time to meet. <br />
                <br />
                {locationOptions.length > 0 && (
                  <span>
                    Click on a location to select it as the place to meet
                  </span>
                )}{" "}
                After, click
